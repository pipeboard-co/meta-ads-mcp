...
export class GHLClient {
  ...
  async getContacts(params: ContactsQueryParams): Promise<GHLContact[]> {
    ...
    const response = await this.client.get<GHLContactsResponse>('/v1/contacts/', {
      params: queryParams,
    });

    let contacts = response.data.contacts || [];

    // Filter by tags if specified (client-side filtering since GHL API doesn't support tag filtering in query)
    if (params.tags && params.tags.length > 0) {
      contacts = contacts.filter((contact) => {
        if (!contact.tags || contact.tags.length === 0) return false;
        // Check if contact has ANY of the specified tags
        return params.tags!.some((tag) => contact.tags!.includes(tag));
      });
    }
  ...
  async getContact(contactId: string): Promise<GHLContact> {
    ...
    const response = await this.client.get<{ contact: GHLContact }>(`/v1/contacts/${contactId}`);
  ...
  async getOpportunities(params: OpportunitiesQueryParams): Promise<GHLOpportunity[]> {
    ...
    const response = await this.client.get<GHLOpportunitiesResponse>('/v1/opportunities/', {
      params: queryParams,
    });
  ...
  async testConnection(): Promise<boolean> {
    try {
      ...
      await this.client.get('/v1/locations/');
      return true;
    } catch (error) {
      return false;
    }
