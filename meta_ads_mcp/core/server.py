...
class StreamableHTTPHandler:
    ...
    def get_auth_config_from_headers(self, request_headers: Dict[str, str]) -> Dict[str, Any]:
        ...
        ALLOWED_VIA_HEADERS = {
            'meta_access_token': True,     # ✅ Primary method - simple and secure
            'meta_app_id': True,           # ✅ Requires OAuth flow - more complex
        ...
        return {
            ...
            'error': {
                ...
                'data': {
                    ...
                    'documentation': 'https://github.com/YOUR_ORG/meta-ads-mcp'
                }
            },
            'id': request_body.get('id')
        }
...
def main():
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog="For more information, see https://github.com/YOUR_ORG/meta-ads-mcp"
    )
    ...
    if meta_access_token:
        logger.info("Using META_ACCESS_TOKEN for direct authentication")
        print("✅ Direct Meta access token provided")
        print(f"   Token preview: {meta_access_token[:10]}...")
    elif meta_app_id and meta_app_secret:
        logger.info("Meta App credentials configured")
        print("✅ Meta App ID and Secret configured")
        print(f"   App ID: {meta_app_id}")
        print("   Use the get_login_link tool to authenticate")
    elif meta_app_id:
        logger.warning("META_APP_ID set but META_APP_SECRET missing")
        print("⚠️  META_APP_ID is set but META_APP_SECRET is missing")
        print("   Long-lived token exchange will not be available")
    else:
        logger.warning("No Meta credentials configured")
        print("⚠️  No Meta credentials configured")
        print("   Set META_APP_ID and META_APP_SECRET environment variables")
        print("   Or provide META_ACCESS_TOKEN for direct authentication")
